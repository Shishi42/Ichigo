...
module.exports = {
  ...
  async run(bot, message, args) {
    ...
    if (args.get("date")){     
      ...
      return await message.reply({ content: `C'est bon, le média sera envoyé le __<t:${Math.floor(datetime) / 1000}:d> à <t:${Math.floor(datetime) / 1000}:T> (<t:${Math.floor(datetime) / 1000}:R>)__.`, ephemeral: true })
    } else {
