    ...
    collector.on('collect', async i => {
      ...
      if (i.customId === 'confirm-post') {
        ...
        new cron.CronJob(prog, async () => {
          message.channel.type == Discord.ChannelType.GuildAnnouncement ? await message.channel.send({ content: `-# <@&${args.get("ping").value}>\n## Un nouveau tournoi arrive à __${args.get("ville").value.toUpperCase()}__`, embeds: [embed], components: []}).then(message => message.crosspost()) : await message.channel.send({ content: `-# <@&${args.get("ping").value}>\n## Un nouveau tournoi arrive à __${args.get("ville").value.toUpperCase()}__`, embeds: [embed], components: []})
