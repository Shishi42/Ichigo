  ...
  async run(bot, message, args) {
    ...
    collector.on('collect', async i => {
      ...
      if (i.customId === 'confirm-team') {
        ...
        let server_emoji = await message.guild.emojis.create({ attachment: msg_emoji.attachments.first().url, name: name.toLowerCase().normalize("NFD").replaceAll('.', '').replaceAll(' ', '_').replaceAll('-', '').replaceAll('(', '').replaceAll(')', '').replaceAll(/[\u0300-\u036f]/g, "") })
