...
module.exports = {
  ...
  async run(bot, team, channel_id, type, logo) {
    ...
    if (type == "affiche") {
      ...
      context.fillText(((team.dataValues.team_desc.split('.').length - 1) >= 1 ? (team.dataValues.team_desc.split('.')[0].join('.').split('!')[0].join('!')).replace(/[^a-zA-Z0-9\sàâäéèêëçùûüôöœæÀÂÄÉÈÊËÇÙÛÜÔÖŒÆ|'.,;:\-()?!"]+/g, '') : team.dataValues.team_desc.replace(/[^a-zA-Z0-9\sàâäéèêëçùûüôöœæÀÂÄÉÈÊËÇÙÛÜÔÖŒÆ|'.,;:\-()?!"]+/g, '')), 48, 250)    
